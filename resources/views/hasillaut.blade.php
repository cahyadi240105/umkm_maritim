        ...
        <div class="max-w-screen-xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 mt-6">
            ...
            <div class="card-order relative max-w-[300px] w-full bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
                ...
                <div class="absolute top-2 left-2 z-20 flex items-center">
                    <input id="checkbox-{{ $index }}" type="checkbox" name="selected_hasils[]" value="{{ $hasil['id'] }}"  class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 focus:ring-2" onclick="handleCheckboxClick(this, {{ $index }})">
                </div>
                ...
                <!-- Gambar -->
                <img src="{{ asset($hasil['gambar']) }}" alt="{{ $hasil['judul'] }}" class="w-full h-[120px] object-cover" />
            
                <!-- Info Produk -->
                <div class="p-4 space-y-2">
                    <h5 class="text-base tracking-tight text-black font-semibold">{{ $hasil['judul'] }}</h5>
                    <p class="text-sm text-black font-bold">Rp.{{ number_format($hasil['harga'], 0, ',', '.') }}</p>
                    <p class="text-sm text-black">
                        <i class="fa-solid fa-location-dot mr-1"></i>{{ $hasil['lokasi'] }}
                    </p>
            
                    <!-- Form Order -->
                    <form action="{{ route('wa.hasillaut', ['id' => $hasil['id']]) }}" method="GET" class="flex items-center justify-between pt-3 border-t border-gray-200 mt-2">
